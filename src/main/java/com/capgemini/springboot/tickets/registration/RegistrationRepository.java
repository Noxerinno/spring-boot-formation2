...
public class RegistrationRepository {
    ...
    public Registration create(Registration registration) {
        ...
        Registration saved = new Registration(id, registration.productId(), ticketCode, registration.attendeeName());
    ...
    public Registration update(Registration registration) {
        ...
        Optional<Registration> opt = findByTicketCode(ticketCode);
        if (opt.isPresent()) {
            Registration existing = opt.get();

            Registration saved = new Registration(existing.id(), existing.productId(), existing.ticketCode(), registration.attendeeName());
            registrationsByTicketCode.put(ticketCode, saved);
            return saved;
        } else {
